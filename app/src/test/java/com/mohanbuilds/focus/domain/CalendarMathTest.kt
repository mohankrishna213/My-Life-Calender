...
class CalendarMathTest {
    ...
    @Test fun `intensity reflects completion ratio`() {
        ...
        val tasks = listOf(Task("t-1", "one", date.toString(), true), Task("t-2", "two", date.toString()))
    ...
    @Test fun `scheduled day with zero completions stays empty grey`() {
        val date = LocalDate.of(2026, 1, 1)
        val tasks = listOf(
            Task("t-1", "one", date.toString()),
            Task("t-2", "two", date.toString()),
            Task("t-3", "three", date.toString()),
        )
    ...
    @Test fun `future recurring occurrence with zero completions is grey in grid map`() {
        ...
        val tasks = listOf(Task("t-1", "daily", date.toString()), Task("t-2", "daily", LocalDate.of(2026, 8, 31).toString()))
